...
function modelGuidePage() {
  ...
  comparisons.push(...[{"name": "WorkBuddy", "vendor": "騰訊", "type": "桌面 AI Agent", "tags": "需求與架構,程式開發與執行", "use": "以自然語言拆解任務，呼叫工具、讀寫本地檔案，涵蓋辦公與程式開發。", "limit": "須安裝並授予相應工具與檔案權限；可用模型與配額依方案。", "image": "本次官方資料未核對圖片輸入細節，請依產品介面確認。", "source": "https://cloud.tencent.cn/product/workbuddy", "sourceLabel": "WorkBuddy 官方介紹", "verified": "2026-09-06"}, {"name": "TRAE", "vendor": "TRAE／字節跳動", "type": "AI 開發工具", "tags": "需求與架構,程式開發與執行", "use": "面向軟體開發，結合程式庫脈絡與 Agent 協作；官方現分 TraeCode 與 TraeWork。", "limit": "開發與辦公產品不同；以實際安裝版本、模型和權限為準。", "image": "本次未核對各版本圖片輸入能力，使用前查看官方文件。", "source": "https://www.trae.ai/", "sourceLabel": "TRAE 官方介紹", "verified": "2026-09-06"}, {"name": "Claude", "vendor": "Anthropic", "type": "AI 對話助手", "tags": "需求與架構,原型與 UI", "use": "透過對話協助拆解問題、整理文件、分析及創作。", "limit": "本欄是 Claude 助手；Claude Code 與 Cowork 是另外的使用方式，不等同預設取得本地專案權限。", "image": "圖片與附件限制依方案和介面；本次不列容量或格式保證。", "source": "https://claude.com/product/overview", "sourceLabel": "Claude 官方介紹", "verified": "2026-09-06"}, {"name": "豆包", "vendor": "字節跳動", "type": "AI 助手", "tags": "需求與架構", "use": "官方定位為 AI 智能助手，可作為學習時的對話入口。", "limit": "本次只核對官方產品定位；未驗證直接修改本地專案、執行測試的能力。", "image": "官方動態頁未提供可核對的圖片規格，本欄暫不作支援承諾。", "source": "https://www.doubao.com/", "sourceLabel": "豆包官方網站", "verified": "2026-09-06"}, {"name": "千問", "vendor": "阿里巴巴", "type": "AI 助手", "tags": "需求與架構", "use": "阿里旗下 AI 助手入口；與 Qwen 模型系列分開理解。", "limit": "本次只核對官方產品定位；具體工具、方案及本地專案操作能力需另外確認。", "image": "本次未取得可核對的圖片規格，依目前產品介面確認。", "source": "https://www.qianwen.com/", "sourceLabel": "千問官方網站", "verified": "2026-09-06"}])
  const modelHeaders = comparisons.map(item => `<th scope="col" data-model-tags="${item.tags}"><strong>${item.name}</strong><br><span class="card-tag">${item.type}</span><br><small>${item.vendor || (item.name.includes("Claude") ? "Anthropic" : "OpenAI")}</small></th>`).join("")
  const comparisonRows = [["主要用途", "use"], ["對應任務", "tags"], ["圖片支援", "image"], ["使用限制", "limit"], ["官方資料", "source"]].map(([label, key]) => `<tr><th scope="row">${label}</th>${comparisons.map(item => `<td data-model-tags="${item.tags}">${key === "source" ? `<a href="${item.source}" target="_blank" rel="noopener noreferrer">${item.sourceLabel} ↗</a><br><small>資料核對：${item.verified || "2026-09-01"}</small>` : key === "tags" ? item.tags.split(",").join("、") : item[key]}</td>`).join("")}</tr>`).join("")
  ...
  return `${navMarkup()}<main class="page-shell"><section class="page-hero"><p class="breadcrumb"><a href="index.html">首頁</a> ／ 模型選擇指南</p><p class="eyebrow">AGENT &amp; MODEL GUIDE</p><h1>我該選哪個 AI 工具與模型？</h1><p class="lead">先分清楚「在哪裡操作」與「由哪個模型理解」，再依任務、權限、限制與官方資料做決定。</p></section>
    ...
    <section class="section"><div class="section-head"><h2>各家 AI 工具與模型比較</h2><p>橫向對照用途、任務、圖片支援與限制。開發 Agent、對話助手與模型分別標示；表格依既有官方資料整理，並非效能排名。手機可左右滑動查看完整表格。</p></div><div class="task-tabs" role="group" aria-label="任務分類"><button class="active" type="button" data-model-task="全部">全部比較</button><button type="button" data-model-task="需求與架構">需求與架構</button><button type="button" data-model-task="原型與 UI">原型與 UI</button><button type="button" data-model-task="程式開發與執行">程式開發與執行</button><button type="button" data-model-task="圖片理解">圖片理解</button></div><div class="model-table-scroll" role="region" aria-label="AI 比較表，可左右捲動" tabindex="0"><table class="model-comparison-table" data-model-table><caption>跨廠商 AI 開發工具、對話助手與模型比較</caption><thead><tr><th scope="col">比較項目</th>${modelHeaders}</tr></thead><tbody>${comparisonRows}</tbody></table></div><p class="policy-note" data-model-empty hidden>目前沒有符合此任務分類的項目。</p></section>
...
if (modelTaskButtons.length) {
  const filterModels = task => {
    ...
    document.querySelectorAll("[data-model-tags]").forEach(row => {
      ...
      row.hidden = !show
      if (show && row.matches("th[scope=col]")) visible += 1
    })
